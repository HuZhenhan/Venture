...
pub(crate) fn app_data_dir(override_dir: Option<&PathBuf>) -> Result<PathBuf> {
    if let Some(dir) = override_dir {
        return Ok(dir.join("Venture"));
    }
...
impl ConfigStore {
    pub async fn load(override_dir: Option<PathBuf>) -> Result<Self> {
        let data_dir = app_data_dir(override_dir.as_ref())?;
    ...
    pub async fn update_provider(
        ...
        let public = {
            ...
            validate_provider_input(&next_base_url, &next_models, next_input)?;
