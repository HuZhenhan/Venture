...
pub fn get_tools_schema() -> Vec<ToolDefinition> {
    vec![
        ...
        task_tool(
            "AskUserQuestion",
            "Ask the user a question and wait for their response. Output STOPS after this call — the system will present the question to the user and wait. Use this whenever you need clarification, choices, or user input to proceed. Supports three modes: choice (with [option]), fill-in-the-blank (with requiresText only), or choice+other (with both [option] and requiresText).",
            json!({
                "type": "object",
                "properties": {
                    "id": {
