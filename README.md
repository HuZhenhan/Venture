...
**[查看项目结构优化报告](./STRUCTURE_OPTIMIZATION_REPORT.md)** | **[完整文档](./docs/README.md)** | **[ 项目架构](./docs/ARCHITECTURE.md)**
...
1. 更加科学的备份回退系统。基于ClaudeCode与GrokBuild两大编码工具的不同回退架构，设计出独特的智能回退管理系统，根据文件热度、占用量智能调节，即避免备份失效又节约空间：
   
   | `small_file_threshold`  | **256 KB** | 小于此值 → 全量快照（存储成本可忽略） |
   | ----------------------- | ---------- | -------------------- |
   | `delta_ratio_threshold` | **0.7**    | 差分压缩比 < 此值时才用增量编码    |
   | `high_frequency_writes` | **3**      | 同一文件修改达此次数 → 倾向增量    |
   | `max_hunk_chain_length` | **20**     | 增量链达此长度 → 强制插入全量锚点   |
